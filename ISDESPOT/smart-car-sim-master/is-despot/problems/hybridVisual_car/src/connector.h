...
public:
	...
	tuple<float,float,float> obstacle;
...
class image_connector {
	...
	string total;
public:
	void sendMessage(COORD& goal_pos, Path& path, message* m) {
	    string message;
	    message += to_string(goal_pos.x) + "," + to_string(goal_pos.y) + ";";
	    message += to_string(get<0>(m->obstacle)) + "," + to_string(get<1>(m->obstacle)) + "," + to_string(get<2>(m->obstacle)) + ";";
