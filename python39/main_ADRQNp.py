...
class Qnetwork():
    def __init__(self, h_size, rnn_cell, myScope):
        ...
        self.imageInput = tf.compat.v1.placeholder(shape=[None, image_input_size], dtype=tf.float32)
        ...
        self.scalarInput = tf.compat.v1.placeholder(shape=[None, input_size], dtype=tf.float32)
        self.weights = tf.compat.v1.get_variable(shape=[input_size, 100], initializer=tf.compat.v1.glorot_normal_initializer(), name=myScope + '_w')
        self.biases = tf.compat.v1.get_variable(shape=[100], initializer=tf.compat.v1.zeros_initializer, name=myScope + '_b')
        self.updatedInput = tf.nn.relu(tf.matmul(self.scalarInput, self.weights) + self.biases)

        self.weights1 = tf.compat.v1.get_variable(shape=[100, var_end_size], initializer=tf.compat.v1.glorot_normal_initializer(), name=myScope + '_w1')
        self.biases1 = tf.compat.v1.get_variable(shape=[var_end_size], initializer=tf.compat.v1.zeros_initializer, name=myScope + '_b1')
        ...
        if use_dueling:
            ...
            self.AW = tf.Variable(tf.random.truncated_normal([(h_size + var_end_size) // 2, num_actions], stddev=0.1), name=myScope + "AW")
            self.VW = tf.Variable(tf.random.truncated_normal([(h_size + var_end_size) // 2, 1], stddev=0.1), name=myScope + "VW")
            ...
            self.weightso1 = tf.compat.v1.get_variable(initializer=tf.random.truncated_normal([h_size + var_end_size, num_actions], stddev=0.1), name=myScope + '_wo1')
            self.biaseso1 = tf.compat.v1.get_variable(shape=[num_actions], initializer=tf.compat.v1.zeros_initializer, name=myScope + '_bo1')
        ...
        self.loss = tf.reduce_mean(input_tensor=self.td_error * self.mask)

        self.trainer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=0.00020, momentum=0.95, epsilon=0.01, decay=0.95)
...
cell = tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=h_size + var_end_size, state_is_tuple=True)
cellT = tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=h_size + var_end_size, state_is_tuple=True)
