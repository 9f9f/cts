...
class InitNetwork:
    def __init__(self, rnn_cell, myScope):
        ...
        self.scalarInput = tf.compat.v1.placeholder(shape=[None, image_input_size], dtype=tf.float32)
        ...
        self.weights = tf.compat.v1.get_variable(shape=[h_size, history_size // 2], initializer=tf.compat.v1.glorot_normal_initializer(),
                                       name=myScope + '_w')
        self.biases = tf.compat.v1.get_variable(shape=[history_size // 2], initializer=tf.compat.v1.zeros_initializer, name=myScope + '_b')
        ...
        self.weightso1 = tf.compat.v1.get_variable(shape=[history_size // 2, 64], initializer=tf.compat.v1.glorot_normal_initializer(),
                                         name=myScope + '_wo1')
        self.biaseso1 = tf.compat.v1.get_variable(shape=[64], initializer=tf.compat.v1.zeros_initializer, name=myScope + '_bo1')
        self.updatedInputOut = tf.nn.relu(tf.matmul(self.rnn1, self.weightso1) + self.biaseso1)

        self.weightsActionOut = tf.compat.v1.get_variable(shape=[64, num_actions], initializer=tf.compat.v1.glorot_normal_initializer(),
                                                name=myScope + '_wActionOut')
        self.biasesActionOut = tf.compat.v1.get_variable(shape=[num_actions], initializer=tf.compat.v1.zeros_initializer,
        ...
        self.predictedAction = tf.argmax(input=self.actionProbabilities, axis=1)

        self.weightsValueOut = tf.compat.v1.get_variable(shape=[64, 1], initializer=tf.compat.v1.glorot_normal_initializer(),
                                               name=myScope + '_wValueOut')
        self.biasesValueOut = tf.compat.v1.get_variable(shape=[1], initializer=tf.compat.v1.zeros_initializer, name=myScope + '_bValueOut')
        self.predictedValue = tf.squeeze(tf.matmul(self.updatedInputOut, self.weightsValueOut) + self.biasesValueOut,
                                         axis=[1])

        self.despot_policy = tf.compat.v1.placeholder(shape=[None, num_actions], dtype=tf.float32)
        self.crossEntropy = tf.nn.softmax_cross_entropy_with_logits(labels=self.despot_policy,
                                                                       logits=self.actionProbabilities)

        self.real_val = tf.compat.v1.placeholder(tf.float32, [None], name='real_val')
